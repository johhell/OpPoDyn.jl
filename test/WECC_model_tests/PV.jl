...
if isdefined(Main, :EXPORT_FIGURES) && Main.EXPORT_FIGURES
    fig = let
        ...
        lines!(ax1, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊pir)).u; label="PowerDynamics pir", color=:blue, linestyle=:dash, linewidth=2)
        lines!(ax1, ref_pv.time, ref_pv[!, Symbol("pV.RenewableGenerator.p.ii")]; label="OpenIPSL pii", color=:red, linewidth=2, alpha=0.7)
        lines!(ax1, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊pii)).u; label="PowerDynamics pii", color=:red, linestyle=:dash, linewidth=2)
        axislegend(ax1)
        ...
        lines!(ax2, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊pvi)).u; label="PowerDynamics pvi", color=:green, linestyle=:dash, linewidth=2)
        lines!(ax2, ref_pv.time, ref_pv[!, Symbol("pV.RenewableGenerator.p.vr")]; label="OpenIPSL pvr", color=:orange, linewidth=2, alpha=0.7)
        lines!(ax2, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊pvr)).u; label="PowerDynamics pvr", color=:orange, linestyle=:dash, linewidth=2)
        axislegend(ax2)
        ...
        lines!(ax3, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊V_t)).u; label="PowerDynamics Vt_in", color=:purple, linestyle=:dash, linewidth=2)
        axislegend(ax3)
        ...
        lines!(ax4, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊P_gen)).u; label="PowerDynamics P_gen", color=:blue, linestyle=:dash, linewidth=2)
        axislegend(ax4)
        ...
        lines!(ax5, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊Q_gen)).u; label="PowerDynamics Q_gen", color=:red, linestyle=:dash, linewidth=2)
        axislegend(ax5)
        ...
        lines!(ax6, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊reecb₊I_pcmd)).u; label="PowerDynamics Ipcmd", color=:green, linestyle=:dash, linewidth=2)
        axislegend(ax6)
        ...
        lines!(ax7, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊reecb₊I_qcmd)).u; label="PowerDynamics Iqcmd", color=:orange, linestyle=:dash, linewidth=2)
        axislegend(ax7)
        ...
        lines!(ax8, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊repca₊Q_ext)).u; label="PowerDynamics Qext", color=:blue, linestyle=:dash, linewidth=2)
        lines!(ax8, ref_pv.time, ref_pv[!, Symbol("pV.PlantController.Pref")]; label="OpenIPSL Pref", color=:red, linewidth=2, alpha=0.7)
        lines!(ax8, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊repca₊P_ref)).u; label="PowerDynamics Pref", color=:red, linestyle=:dash, linewidth=2)
        axislegend(ax8)
        ...
        lines!(ax9, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊pir)).u; label="PowerDynamics pir", color=:blue, linestyle=:dash, linewidth=2)
        axislegend(ax9)
        ...
        lines!(ax10, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊pii)).u; label="PowerDynamics pii", color=:red, linestyle=:dash, linewidth=2)
        axislegend(ax10)
...
fig_pi = let
    ...
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊pir)).u; label="PD PIR", color=Cycled(1), linewidth=2, linestyle=:dash)
    lines!(ax, ref_pv.time, ref_pv[!, "pV.RenewableGenerator.p.ii"]; label="OpenIPSL PII", color=Cycled(2), linewidth=2, alpha=0.5)
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊pii)).u; label="PD PII", color=Cycled(2), linewidth=2, linestyle=:dash)
...
fig_pv = let
    ...
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊pvi)).u; label="PD PVI", color=Cycled(1), linewidth=2, linestyle=:dash)
    lines!(ax, ref_pv.time, ref_pv[!, "pV.RenewableGenerator.p.vr"]; label="OpenIPSL PVR", color=Cycled(2), linewidth=2, alpha=0.5)
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊pvr)).u; label="PD PVR", color=Cycled(2), linewidth=2, linestyle=:dash)
    axislegend(ax; position=:rt)
    fig
end
...
fig_Vt = let
    ...
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊V_t)).u; label="PD Vt", color=Cycled(1), linewidth=2, linestyle=:dash)
    axislegend(ax; position=:rt)
    fig
end
...
fig_Pgen = let
    ...
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊P_gen)).u; label="PD P_gen", color=Cycled(1), linewidth=2, linestyle=:dash)
    axislegend(ax; position=:rt)
    fig
end
...
fig_Qgen = let
    ...
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊Q_gen)).u; label="PD Q_gen", color=Cycled(1), linewidth=2, linestyle=:dash)
    axislegend(ax; position=:rt)
    fig
end
...
fig_Ipcmd = let
    ...
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊reecb₊I_pcmd)).u; label="PD Ipcmd", color=Cycled(1), linewidth=2, linestyle=:dash)
    axislegend(ax; position=:rt)
    fig
end
...
fig_Iqcmd = let
    ...
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊reecb₊I_qcmd)).u; label="PD Iqcmd", color=Cycled(1), linewidth=2, linestyle=:dash)
    axislegend(ax; position=:rt)
    fig
end
...
fig_plant = let
    ...
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊repca₊Q_ext)).u; label="PD Qext", color=Cycled(1), linewidth=2, linestyle=:dash)
    lines!(ax, ref_pv.time, ref_pv[!, "pV.PlantController.Pref"]; label="OpenIPSL Pref", color=Cycled(2), linewidth=2, alpha=0.5)
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊repca₊P_ref)).u; label="PD Pref", color=Cycled(2), linewidth=2, linestyle=:dash)
    axislegend(ax; position=:rt)
    fig
end
...
fig_plant = let
    ...
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊repca₊Voltage_dip)).u; label="PD Voltage_dip plant control", color=Cycled(2), linewidth=2, linestyle=:dash)
    lines!(ax, ts_pv, sol_pv(ts_pv, idxs=VIndex(:GEN1, :PV₊reecb₊Voltage_dip)).u; label="PD Voltage_dip electrical control", color=Cycled(2), linewidth=2, linestyle=:dash)
    axislegend(ax; position=:rt)
    fig
end
